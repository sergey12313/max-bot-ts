...
const debug = debugFactory('one-me:client');
...
export const createClient = (token: string, options: ClientOptions = {}) => {
  ...
  const call = async ({ method, options: callOptions }: CallOptions): Promise<{ status: number; data: any }> => {
...
const getResponseInit = (body?: object | null): RequestInit => {
  if (!body) return {};
...
const buildUrl = (baseUrl: string, path?: Record<string, string | number>): string => {
  let url = baseUrl;
